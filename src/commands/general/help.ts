...
const command: Command = {
  data: {
    name: 'ajuda',
    description: 'Mostra informações sobre todos os comandos ou um comando específico',
  ...
  execute: async (message: Message, args: string[]) => {
    ...
    const command = commands.get(name) || 
                    [...commands.values()].find(cmd => 
                      cmd.data.aliases && cmd.data.aliases.includes(name)
                    );
    ...
    const embed = createEmbed({
      ...
      fields: [
        {
          name: 'Categoria',
          value: command.data.category.charAt(0).toUpperCase() + command.data.category.slice(1),
          inline: true
        }
      ],
      footer: { text: `Solicitado por ${message.author.tag}` },
